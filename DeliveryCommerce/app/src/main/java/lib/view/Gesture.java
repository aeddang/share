...
public class Gesture {
    ...
    private final int changeMin=5;
    private final int changeMax=15;
    private float spdMD=100.f;
	...
	private synchronized boolean moveEvent(ArrayList<Point> locations){
		...
		checkEvent(false);
		//Log.i("t","creat movePosA");
		if(len == startPosA.size()){
			
			
			for(int i=0;i<len;++i){
				location = movePosA.get(i);
			...
			if(Math.abs(change.x)>Math.abs(change.y)){
				 ...
				 if(isHorizontal==true && moveType==1 && len==1){
					 if(delegate!=null){
				 ...
				 if(isVertical==true && moveType==0 && len==1){
					 if(delegate!=null){
	...
	private   synchronized  void  checkEvent(boolean isComplete)
	{
       ...
       if(startPosA.size()==1){
    	   ...
    	   try{
			   ...
			   start=startPosA.get(0);
		       move=movePosA.get(0);
			   //Point change2=changePosA.get(1);
		   ...
		   /*
		   Log.i("t","start :"+start.toString());
		   Log.i("t","start2 :"+start2.toString());
		   
		   Log.i("t","move :"+move.toString());
		   Log.i("t","move2 :"+move2.toString());
		   Log.i("t","startDist :"+startDist+" moveDist: "+moveDist);
		   */
		   float rotate = 0;
