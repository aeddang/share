...
object GraphicUtil
{
        ...
        try {
            ...
            for (i in images.indices) {
                ...
                if (image.javaClass == Bitmap::class.java)
                {
                    ...
                    c.drawBitmap(resizeImage(btm, rec.width(), rec.height()), rec.left.toFloat(), rec.top.toFloat(), null)
                }
    ...
    fun cropBitmapImageByRect(image: Bitmap?, range: Rect): Bitmap? {
        var image = image
        try {
    ...
    fun cropImageByRect(image: Drawable, range: Rect): Bitmap {
    ...
    fun saveImageForAlbum(image: Bitmap, imageName: String, imagePath: String) {
        val cr = ActivityCore.shareInstence()?.getContentResolver()
