...
object DataLoaderUtil
{
    ...
    fun setFileBuffer(file: FileObject, wr: DataOutputStream, delimiter: String)
    {
        ...
        {
            Log.i("DataLoaderUtil", "DataLoader Error FileNotFoundException ")
            return
        }
        ...
        try {
            ...
            wr.writeBytes(cond)
            Log.i("DataLoaderUtil", "cond : $cond")
            wr.writeBytes("\r\n")
            bufferSize = ism!!.available()
            val buffer = ByteArray(bufferSize)
            Log.i("DataLoaderUtil", "buffer : " + buffer.size)
            while (ism!!.read(buffer) != -1) wr.write(buffer, 0, ism!!.read(buffer))
            ism?.close()
        }
        catch (e: IOException)
        {
            Log.i("DataLoaderUtil", "DataLoader Error File write fail")
        }
