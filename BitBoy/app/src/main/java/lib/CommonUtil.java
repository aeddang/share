...
public class CommonUtil 
{
    ...
    public static String getTimeStr (int t,String div)
    {
        int m= t%3600;
        String tim="";
        if(Math.floor((float)t/3600.f)>0){

            tim=intToText((int)Math.floor((float)t/3600.f),2)+div;
        }
        tim=tim+intToText((int)Math.floor((float)m/60.f),2)+ div+intToText(m%60,2) ;
        return tim;
    ...
    public static String getPriceStr(float number)
    {
        return getPriceStr( number,1);
    }
    public static String getPriceStr(float number,int l)
    {
        String str= "";
        if(l==0){
            str= String.format("%f",number);
        }else{
            str= String.format("%."+l+"f",number);
        }

        Log.i("","str : "+str);

        String[] strA = str.split(Pattern.quote("."));
        Log.i("","strA : "+strA.length);
        String  pointStr="";
        if(strA.length >1){
            pointStr=strA[1];
            if(  Integer.parseInt( pointStr ) !=0){
                pointStr="."+pointStr;
            }else{
                pointStr="";
            }
    ...
    public static String getUniqueNameByKey (String key)
    {
        String name=key+getCurrentTimeCode()+Math.random();
        return name;
    }
